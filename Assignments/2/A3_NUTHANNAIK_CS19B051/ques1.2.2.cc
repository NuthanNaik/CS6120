...
TestDeterministic (Ptr<FriisPropagationLossModel> model)
{
  ...
  double txPowerDbm = DbmFromW(50); // dBm
  Ptr<ParabolicAntennaModel> para_ant = CreateObject<ParabolicAntennaModel> ();
  para_ant->SetOrientation(0);
  para_ant->SetBeamwidth(60);
...

  {
    ...
    for (double dist = 200.0; dist <= 2000.0; dist += 200.0)
      {
        b->SetPosition (Vector (dist, 0.0, 0.0));
        double Gt = pow(10,(para_ant->GetGainDb(Angles(a->GetPosition(),b->GetPosition())))/10);
        model->SetSystemLoss(Gt);
        // CalcRxPower() returns dBm.
        double rxPowerDbm = model->CalcRxPower (txPowerDbm, a, b);

        dataset.Add (dist, rxPowerDbm);
