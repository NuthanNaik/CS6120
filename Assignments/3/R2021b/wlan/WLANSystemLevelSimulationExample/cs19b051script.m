...
nodePositions = [0 0 0; 10 0 0; 30 0 0]; % In meters
...
% Configure the STA2
nodeConfig(3).NodePosition = nodePositions(3,:);
nodeConfig(3).TxFormat = 'HE_SU';
nodeConfig(3).BandAndChannel = {[5 36]};
nodeConfig(3).Bandwidth = 20;                    % MHz
nodeConfig(3).MPDUAggregation = true;
nodeConfig(3).TxMCS = 2;
nodeConfig(3).NumTxChains = 1;
nodeConfig(3).DisableRTS = false; 
nodeConfig(3).DisableAck = false;
...
% Configure downlink application traffic
trafficConfig(4).SourceNode = 1;        % AP node ID
trafficConfig(4).DestinationNode = 3;   % STA node ID
trafficConfig(4).DataRateKbps = 100000;
trafficConfig(4).PacketSize = 1500;     % In bytes
trafficConfig(4).AccessCategory = 0;    % Best Effort (0), Background (1), Video (2), and Voice (3)

% Configure uplink application traffic
trafficConfig(3).SourceNode = 3;        % STA node ID
trafficConfig(3).DestinationNode = 1;   % AP node ID
trafficConfig(3).DataRateKbps = 100000;
trafficConfig(3).PacketSize = 1500;     % In bytes
trafficConfig(3).AccessCategory = 0;    % Best Effort (0), Background (1), Video (2), and Voice (3)
...
visualizationInfo.NodeNames = {'AP','STA', 'STA1'};
...
fprintf('Timetaken for simulation = %f seconds /n', diff(datenum([tt1;tt2]))*24*3600);
