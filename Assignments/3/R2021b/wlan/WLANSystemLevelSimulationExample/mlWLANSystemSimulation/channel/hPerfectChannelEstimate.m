...
for nl = 1:Nl
    ...
    pathFiltersT = pathFilters.';
    for np = 1:Np
        h = h + pathFiltersT(:,np).*pathGainsLink(np,:,:,:);
    end
    ...
    ofdmStr.FFTLength = fftSize;
    ofdmStr.NumSymbols = floor((Ns-tstart+1)/(fftSize+cpLength));
    ofdmStr.SymbolOffset = symOffset;
    ofdmStr.CyclicPrefixLength = cpLength;
    for nt = 1:Nt
        rxGrid = comm.internal.ofdm.demodulate(rxWave(tstart:end,:,nt),ofdmStr);
